...
const (
	...
	milestoneNeedsApproval                        // The status/needs-approval label are missing.
	...
	statusMilestoneRemovedLabel = "status/milestone-removed"
	statusLabelsIncompleteLabel = "status/incomplete-labels"
	statusNeedsApprovalLabel    = "status/needs-approval"
	statusNeedsAttentionLabel   = "status/needs-attention"
...
<ul>
 <li>Additional instructions available <a href="https://github.com/kubernetes/community/blob/master/contributors/devel/release/issues.md">here</a></li>
 <li>The commands available for setting labels are documented <a href="https://github.com/kubernetes/test-infra/blob/master/commands.md">here</a></li>
</ul>
</details>
`
...
{{end -}}
{{- if .warnMissingStatus}}
**Action required**: Must specify exactly one of {{.statusLabels}}.
{{end -}}
...
var (
	...
	milestoneStateLabels = []string{
		statusLabelsIncompleteLabel,
		statusNeedsApprovalLabel,
		statusNeedsAttentionLabel,
		statusMilestoneRemovedLabel,
	}
	...
	statusMap = map[string]string{
		"status/in-progress": "Resolution for this issue is in progress.",
		"status/in-review":   "One or more PRs for this issue are in review.",
	}
)
...
func (m *MilestoneMaintainer) issueChange(obj *github.MungeObject) *issueChange {
	...
	//TODO(marun) notify labeled sigs if necessary
	...
	return &issueChange{
		notification: &c.Notification{
			...
			Arguments: strings.TrimSpace(stateConfig.title),
			Context:   strings.TrimSpace(message),
		},
...
func (m *MilestoneMaintainer) issueChangeConfig(obj *github.MungeObject) *issueChangeConfig {
	...
	icc := &issueChangeConfig{
		...
		templateArguments: map[string]interface{}{
			...
			"mode":                m.mode,
			"statusLabels":        formatLabelString(statusMap),
	...
	if kind, priority, sigs, labelErrors := checkLabels(obj.Issue.Labels); len(labelErrors) == 0 {
		...
		if !obj.HasLabel(statusApprovedLabel) {
			if isBlocker {
				...
				removeAfter, ok := gracePeriodRemaining(obj, m.botName, statusNeedsApprovalLabel, m.approvalGracePeriod, time.Now(), false)
				if !ok {
					return nil
				}
		...
		statusLabel, _ := uniqueLabelName(obj.Issue.Labels, statusMap)
		if len(statusLabel) == 0 {
			icc.warnMissingStatus()
		}
		...
		removeAfter, ok := gracePeriodRemaining(obj, m.botName, statusLabelsIncompleteLabel, m.labelGracePeriod, time.Now(), isBlocker)
		if !ok {
			return nil
		}
...
func (icc *issueChangeConfig) summarizeLabels(kind, priority string, sigs []string) {
	icc.enableSection("summarizeLabels")
	icc.state = milestoneCurrent
	quotedSigs := []string{}
	for _, sig := range sigs {
		quotedSigs = append(quotedSigs, quoteLabel(sig))
	}
	arguments := map[string]interface{}{
		"kindLabel":           quoteLabel(kind),
		"kindDescription":     kindMap[kind],
		"priorityLabel":       quoteLabel(priority),
		"priorityDescription": priorityMap[priority],
		"sigLabels":           quotedSigs,
	}
...
func (icc *issueChangeConfig) warnMissingStatus() {
	icc.enableSection("warnMissingStatus")
